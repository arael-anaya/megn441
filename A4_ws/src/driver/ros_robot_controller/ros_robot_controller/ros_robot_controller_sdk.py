...
class PacketControllerState(enum.IntEnum):
    # Format of communication protocol
...
class PacketFunction(enum.IntEnum):
    # Control functions that can be implemented through serial communication
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED control
    PACKET_FUNC_BUZZER = 2  # Buzzer control
    PACKET_FUNC_MOTOR = 3  # Motor control
    PACKET_FUNC_PWM_SERVO = 4  # PWM servo control, from inside to outside on the board are 1-4
    PACKET_FUNC_BUS_SERVO = 5  # Bus servo control
    PACKET_FUNC_KEY = 6  # Obtain button
    PACKET_FUNC_IMU = 7  # Obtain IMU
    PACKET_FUNC_GAMEPAD = 8  # Obtain joystick
    PACKET_FUNC_SBUS = 9  # Obtain aircraft model remote control
    PACKET_FUNC_OLED = 10 # Set OLED display content
    PACKET_FUNC_NONE = 11

class PacketReportKeyEvents(enum.IntEnum):
    # Different status events of the button
...
class Board:
    ...
    def get_imu(self):
        # Get IMU data, and return ax, ay, az, gx, gy, gz
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # Subcommand 0x01 is used to set the SSID. The second byte is the length of the string, which includes the '\0' string termination character
