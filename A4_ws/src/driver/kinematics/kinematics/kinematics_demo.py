...
# @data:2023/03/21
# @author:aiden
# Example of using the kinematics library for a robotic arm
...
###########forward_kinematics##################
fk = ForwardKinematics(debug=True)  # Instantiate forward kinematics and enable printing

print('当前各连杆长度(m):', fk.get_link())  # For specific instruction, please refer to the notes in transform
...
print('input:', pulse)
res = fk.get_fk(pulse)  # Obtain forward kinematics solution
print('output:', res)
...
print('---------------------------------------------------------------------')
###########inverse_kinematics##################
print('当前各连杆长度(m):', get_link())  # For specific instruction, please refer to the notes in transform
...
if res != []:
    pulse = transform.angle2pulse(res)  # Convert to servo pulse width values
    for i in range(len(pulse)):
        print('output%s:'%(i + 1), pulse[i])
else:
    print('no solution')

# Get inverse kinematics by specifying [x, y, z] (m), pitch, and [pitch_min, pitch_max] (deg)
res = get_ik([0.3, 0, 0.3], 0, [-180, 180])
if res != []:
    for i in range(len(res)):
        print('rpy%s:'%(i + 1), res[i][1])  # The corresponding rpy values for each solution
        pulse = transform.angle2pulse(res[i][0])  # Convert to servo pulse width value
        for j in range(len(pulse)):
            print('output%s:'%(j + 1), pulse[j])
else:
    print('no solution')
# Set the length of each link (m) base_link, link1, link2, link3, and tool_link
set_link(0.2, 0.13, 0.13, 0.055, 0.12)  

# Set the range of each joint (deg) joint1, joint2, joint3, joint4, and joint5
